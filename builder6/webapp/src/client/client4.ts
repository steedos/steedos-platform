...
export default class Client4 {
  ...
  token = "";
  csrf = "";
  url = import.meta.env.VITE_B6_ROOT_URL;
  urlVersion = "";
  ...
  acceptInvitation = (spaceId: string, email: string) => {
    ...
    const result: any = this.doFetch<UserProfile>(
      ...
      {
        method: "POST",
        body: JSON.stringify({ email: email, tenantId: spaceId }),
      },
    );
    return result;
  };
  ...
  getSettings = () => {
    return this.doFetch<UserProfile>(`${this.getAccountsRoute()}/settings`, {
      method: "get",
    });
  };

  getMe = () => {
    return this.doFetch<UserProfile>(`${this.getAccountsRoute()}/user`, {
      method: "get",
    });
  };

  getMySpaces = () => {
    return this.doFetch<Space[]>(`${this.getAccountsRoute()}/user/spaces`, {
      method: "get",
    });
  };
