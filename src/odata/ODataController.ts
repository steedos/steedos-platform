...
import { Controller, Body, Get, Post, Put, Delete, Params, QueryParams, CurrentUser, Res } from 'routing-controllers';
...
export class ODataController {
  ...
  async getAll(@CurrentUser({ required: true }) user: { _id: string }, @Params() urlParams: any, @QueryParams() queryParams: any, @Res() response: any) {
    try {
      let userId = user._id;
      ...
      if (permissions.viewAllRecords || (permissions.viewCompanyRecords && getODataManager().isSameCompany(spaceId, userId, createQuery.query.company_id, createQuery.query)) || (permissions.allowRead && userId)) {
        ...
        if (entities) {
          ...
          getODataManager().setHeaders(response);
          return body;
        } else {
          return {
            statusCode: 404,
  ...
  @Get('/api/odata/v4/:spaceId/:objectName/recent')
  async getRecent(@CurrentUser({ required: true }) user: { _id: string }, @Params() urlParams: any, @QueryParams() queryParams: any, @Res() response: any) {
    try {
      let userId = user._id;
      let key = urlParams.objectName;
      let spaceId = urlParams.spaceId;
  ...
  async post(@CurrentUser({ required: true }) user: { _id: string }, @Params() urlParams: any, @Body() bodyParams: any, @Res() response: any) {
    try {
      let userId = user._id;
      let key = urlParams.objectName;
      let spaceId = urlParams.spaceId;
  ...
  async getOne(@CurrentUser({ required: true }) user: { _id: string }, @Params() urlParams: any, @QueryParams() queryParams: any, @Res() response: any) {
    let userId = user._id;
    let key = urlParams.objectName;
    let spaceId = urlParams.spaceId;
    let setErrorMessage = getODataManager().setErrorMessage;
  ...
  async put(@CurrentUser({ required: true }) user: { _id: string }, @Params() urlParams: any, @Body() bodyParams: any, @Res() response: any) {
    try {
      let userId = user._id;
      let key = urlParams.objectName;
      let spaceId = urlParams.spaceId;
  ...
  async remove(@CurrentUser({ required: true }) user: { _id: string }, @Params() urlParams: any, @Body() bodyParams: any, @Res() response: any) {
    try {
      let userId = user._id;
      let key = urlParams.objectName;
      let spaceId = urlParams.spaceId;
