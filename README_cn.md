...
使用传统的命令式编程语言开发企业应用程序极其复杂，计算机如何执行每一步操作，都需要得到精确的指令。使用描述式编程工具，您只需告诉计算机想要的结果，计算机会按照您的需求自动完成编程，生成你想要的企业应用程序。

华炎魔方就是这样一套可视化建模，描述式编程的平台。只需简单的点击鼠标，几乎任何人都可以创建自定义应用程序，实现业务流程自动化。您为企业创建的应用程序可以部署在移动，平板电脑和Web上。华炎魔方把开发难度降低到Excel公式级别，您创建的应用程序可以非常简单，也能功能强大，并可以连接到几乎任何数据源。
...
华炎魔方DX是一套 Visual Studio Code插件，使您能够导出元数据，定义数据导入文件，并轻松指定开发，测试和生产环境的版本，功能和配置选项。有了华炎魔方DX，您可以使用版本管理软件更高效的管理您的代码、您的元数据和系统配置参数。
