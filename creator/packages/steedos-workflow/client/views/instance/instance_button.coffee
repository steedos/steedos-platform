...
instanceButtonHelpers =
	...
	enabled_distribute: ->
		if !Meteor.settings.public?.workflow?.instance_allow_distribute
			return false
		ins = WorkflowManager.getInstance()
		if !ins
			return false
...
#			延迟一毫秒弹出Modal，否则导致loading显示不出来
	...
	'click .btn-workflow-chart': (event, template)->
		...
		ins = WorkflowManager.getInstance()
		flow = db.flows.findOne(ins.flow)
		Steedos.openWindow(Steedos.absoluteUrl("/app/-/page/workflow_chart?instance_id=#{ins._id}&title=#{encodeURIComponent(encodeURIComponent(flow.name))}"),'workflow_chart')
