...
Template.import_users_modal.events
	'change #import-file': (event, template) ->
		module.dynamicImport('xlsx').then (XLSX)->
			items = new Array();
			...
			reader.onload = (ev) ->
				...
				items.forEach (item)->
					if _.has(item, "所属部门")
						item.organization = item["所属部门"]?.toString().trim()
						delete item["所属部门"]
					if _.has(item, "用户名")
						item.username = item["用户名"]?.toString().trim()
						delete item["用户名"]
					if _.has(item, "邮箱")
						item.email = item["邮箱"]?.toString().trim()
						delete item["邮箱"]
					if _.has(item, "姓名")
						item.name = item["姓名"]?.toString().trim()
						delete item["姓名"]
					if _.has(item, "职务")
						item.position = item["职务"]?.toString().trim()
						delete item["职务"]
					if _.has(item, "固定电话")
						item.work_phone = item["固定电话"]?.toString().trim()
						delete item["固定电话"]
					if _.has(item, "手机")
						item.phone = item["手机"]?.toString().trim()
						delete item["手机"]
					if _.has(item, "单位")
						item.company = item["单位"]?.toString().trim()
						delete item["单位"]
					# if _.has(item, "状态")
					# 	item.user_accepted = item["状态"]?.toString().trim()
					# 	delete item["状态"]
					if _.has(item, "排序号")
						item.sort_no = item["排序号"]?.toString().trim()
						delete item["排序号"]
					if _.has(item, "密码")
						item.password = item["密码"]?.toString().trim()
						delete item["密码"]
