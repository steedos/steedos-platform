...
Template.react_grid.onRendered ->
	self = this
	self.autorun (c)->
...
Template.react_grid.helpers Creator.helpers

Template.react_grid.helpers
	objectApiName: ()->
	...
	filters: ()->
		...
		record_id = Template.instance().data.record_id ||  Session.get("record_id")
		return Creator.getListViewFilters(object_name, Template.instance().list_view_id, is_related, related_object_name, record_id)
	columnFields: ()->
		is_related = Template.instance().data.is_related
		object_name = Template.instance().data.object_name
		related_object_name = Template.instance().data.related_object_name
		...
		related_list_item_props = Template.instance().data.related_list_item_props
		selectColumns = Creator.getListviewColumns(curObject, object_name, is_related, list_view_id, related_list_item_props)
		columnFields = [];
		_.each(selectColumns, (fieldApiName)->
			columnFields.push({fieldName: fieldApiName})
		)
		return columnFields;
	hideGridContent: ()->
		is_related = Template.instance().data.is_related
...
Template.react_grid.refresh = (dxDataGridInstance)->
...
Template.react_grid.onDestroyed ->
