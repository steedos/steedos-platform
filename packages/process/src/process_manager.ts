import {
  runProcessAction,
  runProcessNodeAction,
} from "./platform_action_manager";
import { sendNotifications } from "./notifications";
import { SteedosError } from "@steedos/objectql";
import { getHandlersByUserAndRoles } from "./handlers";
...
const getProcessNodeApprover = async (
  instanceId: string,
  processNode: any,
  userSession: any,
  chooseApprover: any,
  ...
  if (isBack) {
    const processInstanceNodes = await objectql
    ...
    if (!_.isEmpty(processInstanceNodes)) {
      ...
      const nodeHistroy = await objectql
    ...
    if (approver === "auto_assign") {
      ...
      if (!_.isEmpty(processNode.assigned_approver_flow_roles)) {
        ...
        if (instanceId) {
          let processInstance = await objectql
        ...
        const handlers = await getHandlersByUserAndRoles(
          submitted_by,
      ...
      if (!_.isEmpty(processNode.assigned_approver_user_field)) {
        if (instanceId) {
          const processInstance = await objectql
            .getObject("process_instance")
            .findOne(instanceId);
          record = await objectql
            .getObject(processInstance.target_object.o)
            .findOne(processInstance.target_object.ids[0]);
        }
        if (record) {
          _.each(
            processNode.assigned_approver_user_field,
            function (fieldName) {
      ...
      if (chooseApprover) {
        if (_.isString(chooseApprover)) {
          ...
          throw new SteedosError(
            "process_approval_error_invalidChooseApprover",
          );
        }
      } else {
...
const addInstanceHistory = async (
  ...
  let instance = await objectql
    .getObject("process_instance")
    .findOne(instanceId);
  let name = "";
  if (options.nodeId) {
    const node = await objectql
      .getObject("process_node")
  ...
  let instanceHistory = await objectql
    .getObject("process_instance_history")
    .insert({
      name: name,
      process_instance: instanceId,
  ...
  if (status === "approved" || status === "rejected") {
    await handleProcessInstance(
      instanceHistory.process_instance,
      status,
      userSession,
    );
  }
  ...
  if (status === "pending") {
    await sendNotifications(instance.created_by, options.actor, {
      instanceHistory,
      status,
      instance,
    });
...
const addInstanceNode = async (
  ...
  const instanceNode = await objectql
    .getObject("process_instance_node")
    .insert({
      process_instance: instanceId,
      process_node: nodeId,
...
const getNextNode = async (
  ...
  if (node) {
    if (node.filtrad) {
      ...
      const canEntry = await objectql.computeFormula(
        node.entry_criteria,
        objectName,
        recordId,
        userSession,
...
//     }
// }
const getPreviousNode = async (
  instanceId: string,
  currentNode: any,
  userSession: any,
) => {
  const previousInstanceNodes = await objectql
    .getObject("process_instance_node")
    .find({
      filters: [
        ["process_instance", "=", instanceId],
  ...
  if (!_.isEmpty(previousInstanceNodes)) {
    ...
    const previousNode = await objectql
      .getObject("process_node")
...
//TODO 处理提交权限
export const getObjectProcessDefinition = async (
  objectName: string,
  recordId: string,
  userSession: any,
) => {
  let spaceId = userSession.spaceId;
  let processes = await objectql.getObject("process_definition").find({
    filters: [
...
export const recordSubmit = async (
  ...
  const pendingInstanceCount = await objectql
    .getObject("process_instance")
    .count({
      filters: [
  ...
  if (!to_final_rejection && !to_final_rejection) {
    ...
    approver = await getProcessNodeApprover(
      null,
      nextNode,
      userSession,
      chooseApprover,
...
export const getReocrdProcessInstance = async (
  objectName: string,
  recordId: string,
  status: string,
  userSession: any,
  ...
  let spaceId = userSession.spaceId;
  return await objectql.getObject("process_instance").find({
    filters: [
      ["space", "=", spaceId],
      ["status", "=", status],
      ["target_object.o", "=", objectName],
...
export const getProcessInstanceWorkitem = async (
  instanceHistoryId: string,
  userSession: any,
) => {
  let spaceId = userSession.spaceId;
  let userId = userSession.userId; //TODO 代理
  const workitme = await objectql.getObject("process_instance_history").find({
    filters: [
...
const handleProcessInstance = async (
  ...
  let otherPendingInstanceNodeCount = await objectql
    .getObject("process_instance_node")
    .count({
      filters: [
        ["process_instance", "=", instanceId],
  ...
  if (otherPendingInstanceNodeCount === 0) {
    const pInstance = await objectql
      .getObject("process_instance")
      .update(instanceId, {
    ...
    if (processStatus === "removed") {
      await unlockObjectRecord(
        pInstance.target_object.o,
        pInstance.target_object.ids[0],
      );
    } else {
      let process = await objectql
    ...
    if (["approved", "rejected"].indexOf(processStatus) > -1) {
      await sendNotifications(pInstance.created_by, pInstance.created_by, {
        status: processStatus,
        instance: pInstance,
      });
    }
...
const getCurrentInstanceNode = async (instanceId) => {
  const pendingNodes = await objectql.getObject("process_instance_node").find({
    filters: [
      ["process_instance", "=", instanceId],
      ["node_status", "=", "pending"],
    ],
...
//TODO 编写trigger 处理elapsed_time
const handleProcessInstanceNode = async (
  instanceId: string,
  currentInstanceNode,
  processStatus: string,
  nextNodeOptions: any,
  ...
  let otherPendingInstanceHistoryCount = await objectql
    .getObject("process_instance_history")
    .count({
      filters: [
        ["process_instance", "=", instanceId],
  ...
  if (otherPendingInstanceHistoryCount === 0) {
    ...
    await objectql.getObject("process_instance_node").updateMany(
      [
        ["process_instance", "=", instanceId],
        ["node_status", "=", "pending"],
      ],
    ...
    if (when) {
      const instance = await objectql
        .getObject("process_instance")
        .findOne(instanceId);
      await runProcessNodeAction(
        currentInstanceNode.process_node,
        when,
        instance.target_object.ids[0],
        userSession,
      );
    }
    ...
    if (nextNodeOptions) {
      ...
      if (to_final_rejection) {
        ...
        await toNextNode(
          instanceId,
          nextNodeOptions.node,
          nextNodeOptions.approve,
          userSession,
        );
      }
    }
...
const getPendingInstanceHistoryCount = async (instanceId: string) => {
  return await objectql.getObject("process_instance_history").count({
    filters: [
      ["process_instance", "=", instanceId],
      ["step_status", "=", "pending"],
    ],
...
const handleProcessInstanceWorkitem = async (
  currentInstanceNode,
  processStatus: string,
  instanceHistoryId: string,
  userSession: any,
  ...
  if (processStatus === "rejected" || processStatus === "approved") {
    ...
    if (instanceHistory.step_node) {
      let processNode = await objectql
        .getObject("process_node")
    ...
    if (
      ...
      (when_multiple_approvers === "unanimous" && processStatus === "rejected")
    ) {
      await objectql.getObject("process_instance_history").updateMany(
        [
  ...
  await handleProcessInstanceNode(
    instanceHistory.process_instance,
    currentInstanceNode,
    processStatus,
    nextNodeOptions,
...
export const processInstanceWorkitemApprove = async (
  instanceHistoryId: string,
  userSession: any,
  comment: string,
  ...
  const nodes = await getProcessNodes(
    instance.process_definition,
    userSession.spaceId,
  );
  const index = _.findIndex(nodes, function (item) {
    return item._id === currentInstanceNode.process_node;
  });
  if (
    currentProcessNode.when_multiple_approvers === "first_response" ||
    (await getPendingInstanceHistoryCount(instanceId)) < 2
  ) {
    let nextNode = await getNextNode(
      nodes,
      index + 1,
      instance.target_object.o,
      instance.target_object.ids[0],
      userSession,
    );
    ...
    if (nextNode) {
      ...
      nextNodeOptions.approve = await getProcessNodeApprover(
        instanceId,
        nextNodeOptions.node,
        userSession,
        chooseApprover,
  ...
  await handleProcessInstanceWorkitem(
    currentInstanceNode,
    "approved",
    instanceHistoryId,
    userSession,
...
export const processInstanceWorkitemReject = async (
  instanceHistoryId: string,
  userSession: any,
  comment: string,
) => {
  ...
  if (currentProcessNode.reject_behavior === "back_to_previous") {
    nextNodeOptions = {};
    nextNodeOptions.node = await getPreviousNode(
      instanceId,
      currentProcessNode,
      userSession,
    );
...
export const processInstanceWorkitemReassign = async (
  instanceHistoryId: string,
  userSession: any,
  comment: string,
  chooseApprover: string,
  ...
  if (!_.isString(chooseApprover)) {
    throw new SteedosError(
      "process_approval_error_reassign_approver_mustBeString",
    );
  }

  const history = await objectql
    .getObject("process_instance_history")
    .update(instanceHistoryId, {
...
export const processInstanceWorkitemRemovebyInstance = async (
  instanceId: string,
  userSession: any,
  comment: string,
) => {
