...
export const sendNotifications = async (
  from: string,
  to: string,
  { instanceHistory = null, instance = null, status },
) => {
  if (!to) {
    return;
  }
  if (!instance) {
    instance = await objectql
      .getObject("process_instance")
      .findOne(instanceHistory.process_instance);
  }
  ...
  var notificationTitle = t(
    `process_notification_${status}_title`,
    { submitter: fromUser.name, recordName: relatedDocName },
    lng,
  );
