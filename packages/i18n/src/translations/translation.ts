...
export const translationApp = function (
  lng: string,
  appId: string,
  app: StringMap,
) {
  ...
  _.each(app.tab_groups, function (tab_group, index) {
    ...
    app.tab_groups[index].group_name = translationTabGroup(
      lng,
      appId,
...
export const getAppTranslationTemplate = function (
  lng: string,
  appId: string,
  _app: StringMap,
) {
  let app = clone(_app);
  let template = {};
  template[getAppNameKey(appId)] = translationAppName(
    lng,
    appId,
    app.label || app.name,
  );
  template[getAppDescriptionKey(appId)] = translationAppDescription(
    lng,
    appId,
    app.description,
  );
  ...
  _.each(app.admin_menus, function (menu) {
    template[getMenuLabelKey(menu._id)] = translationMenuLabel(
      lng,
      menu._id,
