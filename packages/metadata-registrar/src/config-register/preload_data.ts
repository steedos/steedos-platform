...
export const preloadDBObjectFields = async function (datasource: any) {
  ...
  if (datasource.name === "meteor") {
    Creator.Collections[tableName] = Creator.createCollection({
      name: tableName,
    });
  }
...
export const preloadDBObjectButtons = async function (datasource: any) {
  ...
  let buttons: any = await datasource.find(tableName, {
    filters: ["is_enable", "=", true],
  });
...
export const preloadDBRestrictionRules = async function (datasource: any) {
  ...
  for await (const element of records) {
    await registerRestrictionRules.register(
      broker,
      `~database-${tableName}`,
      element,
    );
...
export const preloadDBShareRules = async function (datasource: any) {
  ...
  for await (const element of records) {
    await registerShareRules.register(
      broker,
      `~database-${tableName}`,
      element,
    );
...
export const preloadDBPermissionFields = async function (datasource: any) {
  ...
  if (records && records.length > 0) {
    await registerPermissionFields.mregister(
      broker,
      `~database-${tableName}`,
      records,
    );
...
export const preloadDBPermissionTabs = async function (datasource: any) {
  ...
  let records: any = await datasource.find(tableName, {});
  if (records && records.length > 0) {
    await registerPermissionTabs.mregister(
      broker,
      `~database-${tableName}`,
      records,
    );
