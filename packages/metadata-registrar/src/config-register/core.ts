...
import {
  preloadDBObjectFields,
  preloadDBObjectButtons,
  preloadDBApps,
  preloadDBObjectLayouts,
  preloadDBTabs,
  preloadDBShareRules,
  preloadDBRestrictionRules,
  preloadDBPermissionFields,
  preloadDBPermissionTabs,
} from "./preload_data";
...
export const addOriginalObjectConfigs = function (
  objectName: string,
  datasource: string,
  config: any,
) {
  ...
  _.remove(_original_objectConfigs, {
    name: objectName,
...
const extendOriginalObjectConfig = function (
  objectName: string,
  datasource: string,
  objectConfig: any,
) {
  ...
  let originalObjectConfig = extend(
    ...
    clone(parentOriginalObjectConfig),
    objectConfig,
  );
...
export const addObjectConfigFiles = async (
  filePath: string,
  datasource: string,
  serviceName?: string,
) => {
  ...
  if (!datasource) datasource = "meteor";
...
export const addObjectConfig = async (
  objectConfig: any,
  datasource: string,
  serviceName?: string,
) => {
  ...
  if (object_name === MONGO_BASE_OBJECT || object_name === SQL_BASE_OBJECT) {
    ...
    if (_.isEmpty(parentObjectConfig)) {
      ...
      throw new Error(
        `Object extend failed, object not exist: ${objectConfig.extend}`,
      );
    }
...
export const removeObjectListenerConfig = (_id, listenTo, when) => {
  if (!_id) {
    throw new Error(
      "[config._id] Can not be empty. can not remove object listener.",
    );
  }
  if (!listenTo) {
...
export const loadStandardMetadata = async (
  serviceName: string,
...
export const loadStandardBaseObjects = async (serviceName: string) => {
  if (standardObjectsLoaded) return;
  ...
  const baseTriggers = [
    "base.trigger.js",
    "base.autonumber.trigger.js",
    "base.masterDetail.trigger.js",
    "base.recordFieldAudit.trigger.js",
    "base.recordRecentView.trigger.js",
    "base.tree.trigger.js",
    "base.calendar.trigger.js",
    "base.lookup.trigger.js",
    "base.lockDetail.trigger.js",
  ];
...
export const addRouterConfig = (prefix: string, router: any) => {
  if (!prefix) throw new Error(`Error add router, prefix required`);
...
export function addObjectMethod(
  objectName: string,
  methodName: string,
  method: Function,
) {
  ...
  if (!object)
    throw new Error(
      `Error add method ${methodName}, object not found: ${objectName}`,
    );
...
export function addObjectAction(objectName: string, actionConfig: any) {
  if (!actionConfig.name) throw new Error(`Error add action, name required`);

  let object = getObjectConfig(objectName);
  if (!object)
    throw new Error(
      `Error add action ${actionConfig.name}, object not found: ${objectName}`,
    );
...
export function addObjectPermission(objectName: string, permissionConfig: any) {
  ...
  if (!object)
    throw new Error(
      `Error add permission ${permissionConfig.name}, object not found: ${objectName}`,
    );
