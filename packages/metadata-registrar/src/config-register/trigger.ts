...
export const addObjectListenerConfig = (json: any) => {
  ...
  if (
    !_.isString(json.listenTo) &&
    !_.isFunction(json.listenTo) &&
    !_.isArray(json.listenTo) &&
    !_.isRegExp(json.listenTo)
  ) {
  ...
  if (object_name) {
    ...
    if (object) {
      ...
      try {
        const localObject = getDataSource(object.datasource).getLocalObject(
          object_name,
        );
        if (localObject) {
    ...
    addLazyLoadListeners(
      ...
      Object.assign({}, json, {
        ...
        name: json._id || getMD5(JSONStringify(json)),
      }),
    );
...
export const removePackageObjectTriggers = (serviceName: string) => {
  _.each(_lazyLoadListeners, (listeners, objectName) => {
    ...
    _lazyLoadListeners[objectName] = _.filter(
      listeners,
      function (listener: any) {
        ...
        if (drop) {
          ...
          try {
            const object = getDataSource(
              objectConfig.datasource,
            ).getLocalObject(objectName);
            if (object) {
  ...
  _.each(_patternListerners, (listeners, objectName) => {
    let objectConfig = getObjectConfig(objectName);
    _patternListerners[objectName] = _.filter(
      listeners,
      function (listener: any) {
        const drop = listener.metadataServiceName === serviceName;
        if (drop) {
...
export const loadObjectTriggers = function (
  filePath: string,
  serviceName: string,
) {
  if (!serviceName) {
  ...
  _.each(triggerJsons, (json: any) => {
    triggers.push(
      addObjectListenerConfig(
        Object.assign(json, { metadataServiceName: serviceName }),
      ),
    );
  });
  return triggers;
  ...
  _.each(_patternListerners, (listeners, objectName) => {
    try {
      if (objectName === "*") {
        ...
        try {
          if (
            _.isRegExp(eval(objectName)) &&
            eval(objectName).test(objectApiName)
          ) {
            pushTrigger(listeners);
          }
        } catch (error) {
