...
function getServiceMetadataCacherKey(
  nodeID: string,
  serviceName: string,
  metadataType: string,
...
async function addServiceMetadata(broker, params, meta) {
  ...
  const key = getServiceMetadataCacherKey(
    nodeID,
    metadataServiceName,
    metadataType,
    metadataApiName,
...
async function mget(broker, keys) {
  ...
  const values = await broker.cacher.client.mget(
    ...map(keys, (key) => {
...
async function query(broker, queryKey) {
  try {
    ...
    const keys = await redisScanKeys(
      broker.cacher.client,
      `${keyPrefix}${queryKey}`,
    ); //broker.cacher.client.keys(`${keyPrefix}${queryKey}`); //TODO 此功能仅支持redis cache
...
async function setPackageServices(broker, packageServices) {
  for await (const packageService of packageServices) {
    broker.cacher.set(
      getPackageServiceCacherKey(packageService.nodeID, packageService.name),
  ...
  _.each(services, (serviceItem) => {
    ...
    if (name.startsWith(PACKAGE_SERVICE_PREFIX)) {
      ...
      packageServices.push(
        Object.assign({}, serviceItem, { apiName: `${nodeID}.${name}` }),
      );
    }
  });
...
async function clearPackageServiceMetadatas(
  broker,
  nodeID,
  packageServiceName,
) {
...
async function clearPackageServicesMetadatas(broker, offlinePackageServices) {
  ...
  for await (const packageService of offlinePackageServices) {
    ...
    const clearPackageMetadatas: any = await clearPackageServiceMetadatas(
      broker,
      nodeID,
      name,
    );
  ...
  _.each(
    _.groupBy(clearMetadatas, "metadataType"),
    function (data: any, metadataType) {
...
export async function started(broker) {
  return await broker.cacher.set(
    `${METADATA_SERVICES_PREFIX}.${broker.nodeID}`,
    {},
  );
}
...
export const Register = {
  ...
  async fuzzyDelete(broker: any, key: string) {
    ...
    const keys = await redisScanKeys(
      broker.cacher.client,
      `${keyPrefix}${key}`,
    ); // await broker.cacher.client.keys(`${keyPrefix}${key}`);
  ...
  async refreshServiceMetadatas(broker: any, params: any) {
    ...
    let offlinePackageServicesName = _.difference(
      lastPackageServicesNames,
      packageServicesName,
    );

    let timeoutOfflinePackageServices = _.filter(
      lastPackageServices,
      (lastPackageService) => {
        return (
          lastPackageService &&
    ...
    if (_offlinePackageServices && _offlinePackageServices.length > 0) {
      timeoutOfflinePackageServices = timeoutOfflinePackageServices.concat(
        _offlinePackageServices,
      );
      const onlinePackageServicesName = _.difference(
        lastPackageServices,
        offlinePackageServicesListString,
      );
