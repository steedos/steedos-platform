module.exports = {
    ...
    actions: {
        ...
        standard_approve: {
            ...
            on: "record_only",
            type: "amis_button",
            amis_schema: '{"type":"service","body":[{"type":"button","label":"发起审批","id":"u:6887f3ab860a","editorState":"default","onEvent":{"click":{"weight":0,"actions":[{"actionType":"dialog","dialog":{"type":"dialog","title":"选择流程发起审批","body":[{"type":"form","id":"u:f78efaa51a4f","body":[{"type":"list-select","name":"flowId","label":false,"clearable":true,"id":"u:025b991fd40b","multiple":false,"source":{"method":"get","url":"${context.rootUrl}/api/workflow/v2/get_object_workflows","requestAdaptor":"api.data = {};\\nreturn api;","adaptor":"console.log(\'apyload\', payload);\\nreturn {\\n  data: _.filter(payload, (item) => { return item.object_name == api.body.objectName && item.can_add})\\n};","messages":{},"dataType":"json","headers":{"Authorization":"Bearer ${context.tenantId},${context.authToken}"},"data":{"objectName":"${objectName}","spaceId":"${context.tenantId}"}},"value":"","labelField":"flow_name","valueField":"flow_id","onEvent":{"change":{"weight":0,"actions":[{"actionType":"ajax","outputVar":"responseResult","args":{"options":{},"api":{"url":"${context.rootUrl}/api/object/workflow/drafts","method":"post","requestAdaptor":"api.data = {\\n    \'Instances\': [{\\n        \'flow\': api.body.flowId,\\n        \'applicant\': api.body.context.userId,\\n        \'space\': api.body.context.tenantId,\\n        \'record_ids\': [{ o: api.body.objectName, ids: [api.body.recordId] }]\\n    }]\\n}\\n\\nreturn api;","adaptor":"console.log(\'payload====\', payload);\\nif (payload.error) { \\n  return {\\n    status: 2,\\n    msg: payload.error\\n  }\\n}\\nconst instance = payload.inserts[0];\\nSteedos.openWindow(Steedos.absoluteUrl(\'/app/\' + FlowRouter.current().params.app_id + \'/instances/view/\' + instance._id + \'?display=\' + FlowRouter.current().queryParams.display + \'&side_object=instances&side_listview_id=draft\'))\\nFlowRouter.reload();\\nreturn payload;","messages":{},"headers":{"Authorization":"Bearer ${context.tenantId},${context.authToken}"},"data":{"&":"$$","context":"${context}","objectName":"${objectName}","recordId":"${recordId}"}}},"expression":"${event.data.value}"}]}}}],"wrapWithPanel":false}],"showCloseButton":true,"showErrorMsg":true,"showLoading":true,"className":"","id":"u:ba79188bbf7e","closeOnEsc":true,"actions":[],"size":"md","data":{"&":"$$"},"dataMap":{},"withDefaultData":true,"dataMapSwitch":true,"bodyClassName":"overflow-hidden"}}]}}}],"regions":["body"],"data":{"context":{},"dataComponentId":"","record_id":"","record":{},"permissions":{}},"bodyClassName":"p-0","id":"u:5dd49d3a508c"}'
        },
