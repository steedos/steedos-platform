 ...
 * @LastEditTime: 2023-06-20 16:10:07
...
const beforeDeleteBase = async function () {
    ...
    for (const info of lookupDetailsInfo) {
        ...
        if ('retain' === detailField.deleted_lookup_record_behavior) { // 禁止删除相关记录
            ...
            if (detailDoc) {
                throw new Error(`无法完成您对当前记录的删除尝试，因为它与以下${detailObj.label}关联。：${detailDoc[nameFieldName]}`)
            }
        }
    }
}
