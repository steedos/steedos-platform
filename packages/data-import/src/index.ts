...
export async function getUserSession(
  ...
  try {
    ...
    const spaceUser = await dbManager.findOne(
      "space_users",
      { space: space._id, user: spaceOwner },
      false,
    );
    if (!spaceUser) {
      return;
 ...
 */
export async function preCreateCollection(
  dbManager,
...
export function formatResults(
  ...
  return JSON.parse(
    JSON.stringify(results)
      ...
      .replace(/\${space_owner_id}/g, userId),
  );
}
    ...
    try {
      ...
      for (const result of csvData) {
        for (const record of result.records) {
          const dbRecord = await dbManager.findOne(
            result.objectName,
            { _id: record._id },
            true,
          );
          if (dbRecord) {
            throw new Error(
              `停止导入数据：${result.objectName}对象的${record._id}记录已存在`,
            );
          }
        }
      }
      ...
      for (const result of jsonResult) {
        ...
        const dbRecords = await dbManager.find(
          objectName,
          { _id: { $in: recordsIds } },
          false,
          0,
        ...
        if (dbRecords.length > 0) {
          throw new Error(
            `停止导入数据：${result.objectName}对象已存在${dbRecords.length}条记录`,
          );
        }
      }
    } catch (error) {
