...
async function getSpaceUserProfile(userId: string, spaceId: string) {
  ...
  let spaceUser = await getSteedosSchema()
...
async function getUserRoles(userId: string, spaceId: string) {
  let roles = ["user"];
  let space = await getSteedosSchema()
    .getObject("spaces")
  ...
  let permission_sets = await getSteedosSchema()
...
async function getObjectDataByIds(
  objectName: string,
  ids: string[],
  fields?: string[],
) {
 ...
 * @param userId
 */
export function removeSpaceUserSessionFromCache(
  spaceId: string,
  userId: string,
): void {
...
export async function getSpaceUserSession(spaceId, userId) {
  ...
  if (!spaceSession) {
    ...
    let spaceUsers = await getSteedosSchema()
      .getObject("space_users")
      .directFind({
        filters: `(user eq '${userId}') and (user_accepted eq true)`,
      });
    const findSpaceUser = _.find(spaceUsers, (spaceUser) => {
    ...
    if (su) {
      ...
      let [
        ...
        getObjectDataByIds("organizations", su.organizations, [
          "name",
          "fullname",
          "company_id",
        ]),
      ...
      spaceSession.space = _.find(spaceSession.spaces, (record) => {
        return record._id === userSpaceId;
      });

      spaceSession.companies = companies;
      spaceSession.company = _.find(spaceSession.companies, (record) => {
        return record._id === su.company_id;
      });

      spaceSession.organizations = organizations;
      spaceSession.organization = _.find(
        spaceSession.organizations,
        (record) => {
          return record._id === su.organization;
        },
      );
      ...
      if (spaceSession.companies) {
        spaceSession.company_ids = spaceSession.companies.map(function (
          company: any,
        ) {
          return company._id;
        });
...
async function getSpaces(userSpaceIds: string[]) {
  ...
  for (const sId of userSpaceIds) {
    ...
    if (!cacheDoc) {
      cacheDoc = (
        await getObject("spaces").directFind({
          filters: [["_id", "=", sId]],
 ...
 */
export function deleteSpaceUserSessionCacheByChangedProp(
  newDoc: any,
  oldDoc: any,
): void {
...
export function deleteSpaceCacheByChangedProp(newDoc: any, oldDoc: any): void {
  ...
  if (changed) {
    ...
    const changeAdmins = <string[]>(
      _.difference(newDoc.admins, oldDoc.admins).concat(
        _.difference(oldDoc.admins, newDoc.admins),
      )
    );
