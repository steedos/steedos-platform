...
import {
  ...
  getMetadataTypeInfo,
  getChildMetadataTypeInfos,
  getMetadataTypeInfoByExt,
  getMetadataTypeInfoByDirectory,
  getMetadataTypeInfos,
  getAllowSyncMetadataKeys,
} from "./typeInfo";
...
export function getFileinfoByFilename(filename) {
  ...
  // 使用正则表达式校验 {xxx}.process.{yyy}.{zzz}
    ...
    filename.match(/^[a-zA-Z0-9_-]+\.process\.[a-zA-Z0-9_-]+\.[a-zA-Z0-9_-]+$/)
  ) {
    metadataName = TypeInfoKeys.Process;
    itemName = _.first(filename.split("."));
    ...
    metadataName = TypeInfoKeys.FunctionYML;
    itemName = _.first(filename.split("."));
  } else {
...
export function hasParent(metadataName) {
  const metadataTypeInfo = getMetadataTypeInfo(metadataName);
  return (
    typeof metadataTypeInfo !== "undefined" &&
    typeof metadataTypeInfo.parent != "undefined"
  );
}
...
export function getMetadataTypeObjects(spaceId: string) {
  ...
  _.each(typeInfos, function (info, metadataName) {
    if (
      !_.has(info, "parent") &&
      _.include(allowSyncMetadataKeys, metadataName)
    ) {
