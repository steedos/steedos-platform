...
let tableName = "TestPrimaryKeyForPostgres";
let driver: SteedosPostgresDriver;

describe('primary key autoincrement test for postgres database', () => {
    if (!connectConfig) {
        return true;
    }
    try {
    ...
    let tests = [
        ...
        },
        {
            ...
            runSqls: [`TRUNCATE TABLE "${tableName}" RESTART IDENTITY`],
            expected: {
                eq: undefined
            }
        },
    ...
    before(async () => {
        ...
        // 删除重置主键自增队列
        await driver.run(`TRUNCATE TABLE "${tableName}" RESTART IDENTITY`);
    });
