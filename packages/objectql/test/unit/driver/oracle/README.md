...
- 按官方说明，运行node --server跑服务或pnpm test跑单元测试前，先在cmd命令行设置环境变量`SET PATH=C:\oracle\instantclient_12_1;%PATH%`是有用的（系统的全局变量直接加上这个PATH也是有效果的，我试过也没影响电脑上本身的Oracle客户端连接服务器）
