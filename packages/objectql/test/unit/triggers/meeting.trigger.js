module.exports = {
  ...
  beforeInsert: async function(){
    ...
    var clashRemind = async function(_id,room,start,end){
      var meetings = await getObject("meeting_new").find({
        filters: [
          [
            ['_id', '<>', _id],
            ['room', '=', room]
  ...
  beforeUpdate: async function(){
    ...
    if(doc.start || doc.end){
      ...
      var currentDoc = await this.getObject('meeting_new').findOne(this.id, {fields: ['room', 'start', 'end']})
