...
import {
  SteedosTriggerType,
  SteedosFieldType,
  SteedosFieldTypeConfig,
  SteedosSchema,
  SteedosListenerConfig,
  SteedosObjectListViewTypeConfig,
  SteedosObjectListViewType,
  SteedosIDType,
  SteedosObjectPermissionTypeConfig,
  SteedosActionType,
  SteedosActionTypeConfig,
  SteedosUserSession,
  getSteedosSchema,
} from ".";
import {
  getUserObjectSharesFilters,
  isTemplateSpace,
  isCloudAdminSpace,
...
import {
  SteedosTriggerTypeConfig,
  SteedosTriggerContextConfig,
} from "./trigger";
import { SteedosQueryOptions, SteedosQueryFilters } from "./query";
import {
  SteedosDataSourceType,
  SteedosDatabaseDriverType,
  getDataSource,
} from "./datasource";
import { SteedosFieldDBType } from "../driver/fieldDBType";
import {
  getCurrentObjectFieldFormulasDoc,
  runQuotedByObjectFieldFormulas,
  getCurrentObjectQuotedByFieldFormulaConfigs,
} from "../formula";
import {
  runQuotedByObjectFieldSummaries,
  runCurrentObjectFieldSummaries,
} from "../summary";
...
export class SteedosObjectType extends SteedosObjectProperties {
  ...
  public set enable_audit(value: boolean) {
    if (value && !this._datasource.enable_space) {
      throw new Error(
        `not support, please set ${this._name}.enable_audit to false or remove the enable_audit attribute`,
      );
    }
  ...
  public set enable_instances(value: boolean) {
    if (value && !this._datasource.enable_space) {
      throw new Error(
        `not support, please set ${this._name}.enable_instances to false or remove the enable_instances attribute`,
      );
    }
  ...
  public set enable_trash(value: boolean) {
    if (value && !this._datasource.enable_space) {
      throw new Error(
        `not support, please set ${this._name}.enable_trash to false or remove the enable_trash attribute`,
      );
    }
  ...
  public set enable_share(value: boolean) {
    if (value && !this._datasource.enable_space) {
      throw new Error(
        `not support, please set ${this._name}.enable_share to false or remove the enable_share attribute`,
      );
    }
  ...
  private async callMetadataObjectServiceAction(action, params?) {
    ...
    const result = await this.schema.metadataBroker.call(
      actionFullName,
      params,
    );
    return result;
  ...
  private checkField() {
    let driverSupportedColumnTypes =
      this._datasource.adapter.getSupportedColumnTypes();
    _.each(this.fields, (field: SteedosFieldType, key: string) => {
      if (
        SteedosFieldDBType[field.fieldDBType] &&
        !driverSupportedColumnTypes.includes(field.fieldDBType)
      ) {
        throw new Error(
          `driver ${this._datasource.driver} can not support field ${key} config`,
        );
      }
    });
  }
  ...
  constructor(
    ...
    config: SteedosObjectTypeConfig,
  ) {
    super();
    ...
    if (/^[_a-zA-Z][_a-zA-Z0-9]*$/.test(object_name) != true) {
      throw new Error(
        "invalid character, object_name can only be start with _ or a-zA-Z and contain only _ or _a-zA-Z0-9. you can set table_name",
      );
    }
    ...
    if (
      this._datasource.driver == SteedosDatabaseDriverType.Mongo ||
      this._datasource.driver == SteedosDatabaseDriverType.MeteorMongo
    ) {
      this._idFieldName = "_id";
    }

    this.schema.setObjectMap(this.name, {
  ...
  getConfig() {
    return (
      this.datasource.getCacheObject(this.name)?.metadata ||
  ...
  registerTrigger(trigger: SteedosTriggerType) {
    ...
    if (
      !_.isString(this._datasource.driver) ||
      this._datasource.driver != SteedosDatabaseDriverType.MeteorMongo ||
  ...
  private async runTirgger(
    trigger: SteedosTriggerType,
      ...
      _.map(_triggers, (item) => {
        if (item && item.metadata) {
          ...
          if (
            metadata.isEnabled &&
            (metadata.when === when ||
              (isArray(metadata.when) && includes(metadata.when, when)))
          ) {
            if (metadata.isPattern) {
              try {
                if (metadata.listenTo === "*") {
                  ...
                  _.isArray(metadata.listenTo) &&
                  _.include(metadata.listenTo, this.name)
                ) {
                  triggers.push(item);
                } else if (
                  _.isRegExp(metadata.listenTo) &&
                  metadata.listenTo.test(this.name)
                ) {
                  triggers.push(item);
                } else if (
                  _.isString(metadata.listenTo) &&
                  metadata.listenTo.startsWith("/")
                ) {
                  try {
                    if (
                      _.isRegExp(eval(metadata.listenTo)) &&
                      eval(metadata.listenTo).test(this.name)
                    ) {
                      triggers.push(item);
                    }
                  } catch (error) {}
  ...
  async runFunctionTriggers(
    ...
    for (const trigger of triggers) {
      ...
      const result = await runTriggerFunction(
        trigger.metadata,
        {
          getObject: getObject,
  ...
  async runTriggerActions(when: string, context: SteedosTriggerContextConfig) {
    ...
    for (const trigger of triggers) {
      let params = generateActionParams(when, context);
      const result = await this._schema.metadataBroker
        .call(`${trigger.service.name}.${trigger.metadata.action}`, params)
        .catch((error) => {
  ...
  toConfig() {
    ...
    if (this.list_views) {
      ...
      _.each(
        this.list_views,
        (list_view: SteedosObjectListViewType, key: string) => {
  ...
  async getDetailPaths() {
    return await this.callMetadataObjectServiceAction(`getDetailPaths`, {
      objectApiName: this.name,
    });
  }

  async getMasterPaths() {
    return await this.callMetadataObjectServiceAction(`getMasterPaths`, {
      objectApiName: this.name,
    });
  }

  async getMaxDetailsLeave(paths?) {
    return await this.callMetadataObjectServiceAction(`getMaxDetailsLeave`, {
      objectApiName: this.name,
      paths,
    });
  }
  ...
  async getMaxMastersLeave(paths?) {
    return await this.callMetadataObjectServiceAction(`getMaxMastersLeave`, {
      objectApiName: this.name,
      paths,
    });
  }
  ...
  setListView(list_view_name: string, config: SteedosObjectListViewTypeConfig) {
    this.list_views[list_view_name] = new SteedosObjectListViewType(
      list_view_name,
      this,
      config,
    );
  ...
  setAction(action_name: string, actionConfig: SteedosActionTypeConfig) {
    this._actions[action_name] = new SteedosActionType(
      action_name,
      this,
      actionConfig,
    );
   ...
   * @param userSession
   */
  private handlerDuplicateKeyError(
    error: Error,
  ...
  getObjectRolesPermission(spaceId?: string) {
    let globalPermission = this._datasource.getObjectRolesPermission(
      this._name,
    );
    if (spaceId) {
      let permission = this._datasource.getObjectSpaceRolesPermission(
        this._name,
        spaceId,
      );
  ...
  async getUserObjectPermission(
    ...
    if (rolesFieldsPermission === false) {
      ...
      if (!rolesFieldsPermission) {
        rolesFieldsPermission =
          await FieldPermission.getObjectFieldsPermissionGroupRole(this.name);
      }
    }
          ...
          if (_.isBoolean(v)) {
            ...
            if ("modifyAssignCompanysRecords" === k) {
              if (!_.isEmpty(_v) && _.isArray(_v)) {
                ...
                // 去重
                userObjectPermission["viewAssignCompanysRecords"] = _.uniq(
                  userObjectPermission["viewAssignCompanysRecords"],
                );
              }
            }
            if (!_.isEmpty(_v) && _.isArray(_v)) {
    ...
    if (isTemplateSpace(spaceId)) {
      return Object.assign({}, userObjectPermission, {
        allowRead: true,
        viewAllRecords: true,
  ...
  async directAggregatePrefixalPipeline(
    query: SteedosQueryOptions,
    prefixalPipeline: any[],
    userSession?: SteedosUserSession,
  ) {
    let clonedQuery = Object.assign({}, query);
    if (userSession)
  ...
  async updateMany(
    queryFilters: SteedosQueryFilters,
    doc: Dictionary<any>,
    userSession?: SteedosUserSession,
  ) {
    doc = this.formatRecord(doc);
  ...
  async getAbsoluteUrl(app_id, record_id?) {
    ...
    if (record_id)
      return absoluteUrl(
        "/app/" + app_id + "/" + object_name + "/view/" + record_id,
      );
    else {
      return absoluteUrl(
        "/app/" + app_id + "/" + object_name + "/grid/" + list_view_id,
      );
    }
  }
  ...
  async getDetails() {
    return await this.callMetadataObjectServiceAction(`getDetails`, {
      objectApiName: this.name,
    });
  }

  async getMasters() {
    return await this.callMetadataObjectServiceAction(`getMasters`, {
      objectApiName: this.name,
    });
  }

  async getLookupDetails() {
    return await this.callMetadataObjectServiceAction(`getLookupDetails`, {
      objectApiName: this.name,
    });
  }

  async getDetailsInfo() {
    return await this.callMetadataObjectServiceAction(`getDetailsInfo`, {
      objectApiName: this.name,
    });
  }

  async getMastersInfo() {
    return await this.callMetadataObjectServiceAction(`getMastersInfo`, {
      objectApiName: this.name,
    });
  }

  async getLookupDetailsInfo() {
    return await this.callMetadataObjectServiceAction(`getLookupDetailsInfo`, {
      objectApiName: this.name,
    });
  }
  ...
  async getRelationsInfo() {
    return await this.callMetadataObjectServiceAction(`getRelationsInfo`, {
      objectApiName: this.name,
    });
  }
  ...
  getAccessFields(objectFields, objectLayout, objectPermission) {
    ...
    const universallyRequiredFieldsName = _.pluck(
    ...
    _.each(objectFields, (field) => {
      if (field) {
        ...
        let fieldLayout =
          objectLayout && objectLayout.fields
            ? _.find(objectLayout.fields, (item) => {
                return item.field_name == field.name;
              })
            : null;

        const isUniversallyRequiredField = _.contains(
          universallyRequiredFieldsName,
          field.name,
        );
    ...
    if (objectLayout) {
      ...
      _.each(universallyRequiredFieldsName, function (fieldName) {
        const fieldLayout = _.find(objectLayout.fields, (item) => {
  ...
  private async getContext(userSession, defContext) {
    ...
    if (!objectConfig) {
      const pAll = [];
      pAll.push(
        this.callMetadataObjectServiceAction("get", {
          objectApiName: this.name,
        }),
      );
      pAll.push(
        getObjectLayouts(userSession.profile, userSession.spaceId, this.name),
      );
      pAll.push(
        getObject("process_definition").find({
          filters: [
      ...
      pAll.push(
        getObject("object_listviews").directFind({
          filters: [
            ["space", "=", userSession.spaceId],
            ["object_name", "=", this.name],
            [
  ...
  async getRecordView(userSession, context: any = {}) {
    try {
      ...
      if (global["HAS_LICENSE_SERVICE"]) {
        versionsMap = {
          [PLATFORM_ENTERPRISE]: await broker.call(
            `${LICENSE_SERVICE}.isPlatformEnterprise`,
            { spaceId: userSession.spaceId },
          ),
          [PLATFORM_PROFESSIONAL]: await broker.call(
            `${LICENSE_SERVICE}.isPlatformProfessional`,
            { spaceId: userSession.spaceId },
          ),
        };
      }
      let {
        ...
        rolesFieldsPermission,
        relationsInfo,
      } = await this.getContext(userSession, context);
      const lng = userSession.language;
      const profile = userSession.profile;
      const profileDoc = await broker.call(
        "@steedos/service-cachers-manager.getProfile",
        { name: profile, spaceId: userSession.spaceId },
      );
      ...
      objectConfig.permissions = await this.getUserObjectPermission(
        userSession,
        rolesFieldsPermission,
      );
      ...
      if (layouts && layouts.length > 0) {
        ...
        const layoutButtonsName = _.uniq(
          _.pluck(objectLayout.buttons, "button_name").concat(
            defaultStandardButtons,
          ),
        );
        ...
        _.each(objectConfig.actions, function (action) {
          if (
            action.name.startsWith("standard_") &&
            !_.include(defaultStandardButtons, action.name)
          ) {
      ...
      objectConfig.fields = this.getAccessFields(
        objectConfig.fields,
        objectLayout,
        objectConfig.permissions,
      );
      ...
      const listViews = {};
      _.map(
        _.sortBy(_.values(objectConfig.list_views), "sort_no"),
        (listView: any) => {
      ...
      if (objectConfig && objectConfig.details && objectConfig.details.length) {
        ...
        for (let i = 0; i < result.length; i++) {
          ...
          const relatedObjectPermissions = await broker.call(
      ...
      if (
        objectConfig &&
        objectConfig.related_list &&
        objectConfig.related_list.length
      ) {
  ...
  async getDefaultRecordView(userSession) {
    ...
    const objectConfig: any = await this.callMetadataObjectServiceAction(
      "getOriginalObject",
      { objectApiName: this.name },
    );
    let sortedFields = [];
    ...
    sortedFields = sortBy(sortedFields, function (o) {
      return o.sort_no;
    });
    ...
    for await (const related of relatedLists) {
      if (related) {
        ...
        const relatedObjectAllListView = _.find(
          relatedObject.list_views,
          function (listview) {
  ...
  async createDefaultRecordView(userSession) {
    ...
    try {
      ...
      return await getObject("object_layouts").insert(
        Object.assign({}, defaultRecordView, { name, label, profiles }),
        userSession,
      );
    } catch (error) {
  ...
  async getRelateds() {
    ...
    for (const info of relatedInfos) {
      if (!info.startsWith("__")) {
        ...
        let related_field =
          await getObject(detailObjectApiName).getField(detailFieldName);
        if (related_field) {
          if (
            (related_field.type == "master_detail" ||
            ...
            if (detailObjectApiName == "object_fields") {
              ...
              related_objects.push({
                ...
                write_requires_master_read:
                  related_field.write_requires_master_read,
              });
            }
          }
        }
  ...
  private async runBeforeTriggers(
    method: string,
    context: SteedosTriggerContextConfig,
  ) {
  ...
  private async getTriggerContext(when: string, method: string, args: any[]) {
    ...
    let context: SteedosTriggerContextConfig = {
  ...
  private async processUnreadableField(
    userSession: SteedosUserSession,
    query: SteedosQueryOptions,
  ) {
    if (!userSession) {
  ...
  private async getInsertBaseDoc(
    doc: Dictionary<any>,
    userSession?: SteedosUserSession,
  ) {
    ...
    if (
      ...
      driver == SteedosDatabaseDriverType.MeteorMongo
    ) {
  ...
  private async getUpdateBaseDoc(
    doc: Dictionary<any>,
    userSession?: SteedosUserSession,
  ) {
    let driver = this._datasource && this._datasource.driver;
  ...
  private async callAdapter(method: string, ...args: any[]) {
    ...
    //共享规则
    const {
      rulesFilters: shareRuleFilters,
      allowEditRulesFilters: allowEditShareRulesFilters,
    } = await ShareRules.getUserObjectFilters(this.name, userSession);

    // 判断处理工作区权限，公司级权限，owner权限
    if (
      !(
        this.enable_space_global === true &&
        _.include(["find", "findOne"], method)
      )
    ) {
      if (!_.isEmpty(userSession) && this._datasource.enable_space) {
        this.dealWithFilters(method, args);
        await this.dealWithMethodPermission(
          method,
          args,
          shareRuleFilters,
          allowEditShareRulesFilters,
    ...
    if (this.isDirectCRUD(method)) {
      ...
      let docAfterFormulaRun = await this.getRecordFormulaDoc(
        method,
        objectName,
        doc,
        previousDoc,
      ...
      if (
        ...
        if (method === "count") {
          ...
          if (userSession) {
            ...
            await this.appendRecordPermission(
              _records,
              allowEditShareRulesFilters,
              userSession,
            );
      ...
      if (
        ...
        if (
          _.isEmpty(afterTriggerContext.data) ||
      ...
      if (returnValue) {
        ...
        await this.runRecordQuotedByObjectFieldFormulas(
          method,
          objectName,
          recordId,
          doc,
   ...
   */
  private async getRecordFormulaDoc(
    method: string,
    objectName: string,
    doc: any,
    previousDoc: any,
    ...
    if (["insert", "update"].indexOf(method) > -1) {
      ...
      let fieldNames =
        method === "update" ? getFieldNamesFromDoc(doc, this.fields) : null;
      let quotedByConfigs: any;
      if (fieldNames) {
        quotedByConfigs = await getCurrentObjectQuotedByFieldFormulaConfigs(
          objectName,
          fieldNames,
        );
      }
      ...
      setDoc = await getCurrentObjectFieldFormulasDoc(
        objectName,
        formulaDoc,
        userSession,
        quotedByConfigs,
   ...
   */
  private async runRecordQuotedByObjectFieldFormulas(
    method: string,
    objectName: string,
    recordId: string,
    doc: any,
    ...
    if (["update", "delete"].indexOf(method) > -1) {
      ...
      if (method === "update") {
        ...
        if (
          _.intersection(["$inc", "$min", "$max", "$mul"], updatedDocFieldNames)
            .length
        ) {
   ...
   * 增删改记录时，重算并更新汇总字段值
   */
  private async runRecordSummaries(
    method: string,
    objectName: string,
    recordId: string,
    doc: any,
    ...
    if (["insert", "update", "delete"].indexOf(method) > -1) {
      ...
      await runQuotedByObjectFieldSummaries(
        objectName,
        recordId,
        previousDoc,
        userSession,
  ...
  private getObjectEditPermissionFilters(
    ...
    if (!_.isEmpty(objectPermission.modifyAssignCompanysRecords)) {
      objectPermissionFilters.push(
        `(${formatFiltersToODataQuery([["company_id", "in", objectPermission.modifyAssignCompanysRecords], "or", ["company_ids", "in", objectPermission.modifyAssignCompanysRecords]], userSession)})`,
      );
    }
    ...
    if (allowEditShareRulesFilters && allowEditShareRulesFilters.length > 0) {
      objectPermissionFilters.push(
        `(${allowEditShareRulesFilters.join(" or ")})`,
      );
    }
  ...
  private async dealWithMethodPermission(
    method: string,
    args: any[],
    shareRuleFilters,
    allowEditShareRulesFilters,
    ...
    if (userSession) {
      ...
      if (
        ...
        if (
          this.table_name == "cfs.files.filerecord" ||
        ...
        let spaceFilter,
          ...
          ownerFilter,
          sharesFilter,
          restrictionRuleFilters,
          clientFilter = query.filters,
          filters,
          permissionFilters = [],
          userFilters = [];
        ...
        if (!objPm.viewAllRecords && objPm.viewAssignCompanysRecords) {
          _.each(objPm.viewAssignCompanysRecords, (assignCompanyId) => {
            viewAssignCompanysRecordsFilter.push(
              `((company_id eq '${assignCompanyId}') or (company_ids eq '${assignCompanyId}'))`,
            );
          });
        }
        if (!_.isEmpty(viewAssignCompanysRecordsFilter)) {
          permissionFilters.push(
            `(${viewAssignCompanysRecordsFilter.join(" or ")})`,
          );
        }

        if (!_.isEmpty(shareRuleFilters) && !objPm.viewAllRecords) {
          // 如果用户没有查看所有的权限，才考虑共享规则；如有查看所有的权限，则无需考虑共享规则。
          permissionFilters.push(`(${shareRuleFilters.join(" or ")})`);
        }

        // 限制规则
        restrictionRuleFilters = await RestrictionRule.getUserObjectFilters(
          this.name,
          userSession,
        );
        ...
        if (!_.isEmpty(userFilters)) {
          filters = filters
            ? `(${filters}) and (${userFilters.join(" and ")})`
        ...
        if (!objPm.modifyAllRecords && objPm.modifyCompanyRecords) {
          ...
          if (companyFilters) {
            if (_.isString(id)) {
              id = {
                filters: `(_id eq \'${id}\') and (${companyFilters}${objectPermissionEditFilters})`,
              };
              ...
              id.filters = id.filters
                ? `(${id.filters}) and (${companyFilters}${objectPermissionEditFilters})`
                : `(${companyFilters}${objectPermissionEditFilters})`;
            }
          }
        } else if (
          !objPm.modifyAllRecords &&
          !objPm.modifyCompanyRecords &&
          objPm.allowEdit
        ) {
          if (_.isString(id)) {
            id = {
              filters: `(_id eq \'${id}\') and (owner eq \'${userId}\' ${objectPermissionEditFilters})`,
            };
          } else if (_.isObject(id)) {
            if (id.filters && !_.isString(id.filters)) {
              id.filters = formatFiltersToODataQuery(id.filters);
            ...
            id.filters = id.filters
              ? `(${id.filters}) and (owner eq \'${userId}\' ${objectPermissionEditFilters})`
              : `(owner eq \'${userId}\' ${objectPermissionEditFilters})`;
          }
        } else if (!objPm.modifyAllRecords && objectPermissionEditFilters) {
          objectPermissionEditFilters = objectPermissionEditFilters.replace(
            " or ",
            "",
          );
          if (_.isString(id)) {
            id = {
              filters: `(_id eq \'${id}\') and (${objectPermissionEditFilters})`,
            };
          } else if (_.isObject(id)) {
            if (id.filters && !_.isString(id.filters)) {
              id.filters = formatFiltersToODataQuery(id.filters);
            ...
            id.filters = id.filters
              ? `(${id.filters}) and (${objectPermissionEditFilters})`
        ...
        if (!objPm.modifyAllRecords && objPm.modifyCompanyRecords) {
          ...
          if (companyFilters) {
            ...
            queryFilters = queryFilters
              ? `(${queryFilters}) and (${companyFilters})`
              : `(${companyFilters})`;
