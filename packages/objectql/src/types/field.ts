...
export class SteedosFieldType
  extends SteedosFieldProperties
  implements Dictionary
{
  ...
  constructor(
    ...
    if (this.type === "formula" && !this.data_type) {
      throw new Error(
        `${this._object.name}.${this.name} invalid field type formula, miss data_type property`,
      );
    }
  ...
  getIndexInfo = () => {
    ...
    if (
      this.index ||
      this.unique ||
      defaultIndexFieldTypes.includes(this.type)
    ) {
      let index = {};
      ...
      if (partialFilterExpression) {
        indexInfo.partialFilterExpression = partialFilterExpression;
      }
      return indexInfo;
    }
  };
      ...
      if (datasource.name != "meteor") {
        if (_.isString(this.reference_to)) {
          ...
          _.each(this.reference_to, (_reference_to) => {
            reference_to.push(
              this.transformReferenceTo(_reference_to, datasource),
            );
          });
