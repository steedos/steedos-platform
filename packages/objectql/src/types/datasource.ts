...
export type SteedosDataSourceTypeConfig = {
  ...
  objectFiles?: string[];
  objectsRolesPermission?: Dictionary<
    Dictionary<SteedosObjectPermissionTypeConfig>
  >;
...
export class SteedosDataSourceType implements Dictionary {
  ...
  private metadataObjectsName?: Record<string, any> = {};
  private _metadataAdapter: SteedosDriver;
  ...
  private _objectsRolesPermission: Dictionary<
    Dictionary<SteedosObjectPermissionType>
  > = {};
  private _objectsSpaceRolesPermission: Dictionary<
    Dictionary<Dictionary<SteedosObjectPermissionType>>
  > = {};
  ...
  async flushCacheObjects() {
    this.cacheObjects = await this.schema.metadataRegister.getObjectsConfig(
      this.name,
    );
  }
  ...
  async setObject(
    ...
    const findIndex = _.findIndex(this.cacheObjects, (item: any) => {
      return item?.metadata?.name === objectApiName;
    });

    if (findIndex != -1) {
      this.cacheObjects[findIndex] = Object.assign(
        { service: { name: serviceName } },
        { metadata: objectConfig },
      );
    } else {
      this.cacheObjects.push(
        Object.assign(
          { service: { name: serviceName } },
          { metadata: objectConfig },
        ),
  ...
  initDriver() {
    ...
    if (this.name === "default") {
      this._metadataAdapter = new MetadataDriver(driverConfig);
    }
  ...
  constructor(
    ...
    config: SteedosDataSourceTypeConfig,
    schema: SteedosSchema,
  ) {
    ...
    if (_.has(config, "enable_space")) {
      ...
      if (
        this._driver == SteedosDatabaseDriverType.MeteorMongo ||
  ...
  setObjectPermission(
    object_name: string,
    objectRolePermission: SteedosObjectPermissionTypeConfig,
  ) {
    ...
    this._objectsRolesPermission[object_name][objectRolePermission.name] =
      new SteedosObjectPermissionType(object_name, objectRolePermission);
  }
  ...
  getObjectSpaceRolesPermission(object_name: string, spaceId: string) {
    ...
    _.each(
      _.filter(cacherPermissionObjects[spaceId], function (objectPermission) {
        return objectPermission.object_name === object_name;
      }),
      (item) => {
        permission[item.name] = new SteedosObjectPermissionType(
          object_name,
          item,
        );
      },
  ...
  removeObjectSpacePermission(
    object_name: string,
    spaceId: string,
    objectRolePermissionName: string,
  ) {
    if (
      this._objectsSpaceRolesPermission[object_name] &&
      this._objectsSpaceRolesPermission[object_name][spaceId]
    ) {
      delete this._objectsSpaceRolesPermission[object_name][spaceId][
        objectRolePermissionName
      ];
    }
  }
  ...
  async aggregate(
    ...
    return await this.getAdapter(tableName).aggregate(
      tableName,
      query,
      externalPipeline,
      userId,
  ...
  async findOne(
    ...
    return await this.getAdapter(tableName).findOne(
      tableName,
      id,
      query,
      userId,
  ...
  async updateOne(
    ...
    return await this.getAdapter(tableName).updateOne(
      tableName,
      id,
      doc,
      userId,
  ...
  async updateMany(
    ...
    return await this.getAdapter(tableName).updateMany(
      tableName,
      queryFilters,
      doc,
      userId,
  ...
  async directFind(
    ...
    return await this.getAdapter(tableName).directFind(
      tableName,
      query,
      userId,
    );
  ...
  async directInsert(
    ...
    return await this.getAdapter(tableName).directInsert(
      tableName,
      doc,
      userId,
    );
  ...
  async directUpdate(
    ...
    return await this.getAdapter(tableName).directUpdate(
      tableName,
      id,
      doc,
      userId,
  ...
  async directAggregate(
    ...
    return await this.getAdapter(tableName).directAggregate(
      tableName,
      query,
      externalPipeline,
      userId,
  ...
  async directAggregatePrefixalPipeline(
    ...
    return await this.getAdapter(tableName).directAggregatePrefixalPipeline(
      tableName,
      query,
      prefixalPipeline,
      userId,
  ...
  async directUpdateMany(
    ...
    return await this.getAdapter(tableName).directUpdateMany(
      tableName,
      queryFilters,
      doc,
      userId,
  ...
  initObjectPermissions() {
    _.each(
      this.config.objectsRolesPermission,
      (objectRolesPermission, object_name) => {
  ...
  async connect() {
    // this.initObjects();
    // init typeorm
    if (this._adapter.connect) await this._adapter.connect();
    // init typeorm
    if (this._adapter.init) await this._adapter.init(this._objects);
  }

  async close() {
    if (this._adapter?.close) this._adapter.close();
  }
