...
function connectAndMonitor(client) {
    ...
    client.on('commandSucceeded', (event) => {
        ...
        if (duration >= SLOW_QUERY_THRESHOLD_MS) {
            console.warn(`🚨 Mongodb 慢查询检测到 (成功):`);
    ...
    async connect() {
        if (!this._client) {
            if (process.env.STEEDOS_CSFLE_MASTER_KEY) {
                ...
                this._client = await MongoClient.connect(this._url, {useNewUrlParser: true, useUnifiedTopology: true, monitorCommands: STEEDOS_MONGODB_MONITOR_COMMANDS, });
            }
