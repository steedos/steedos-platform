import { getStringTimeValue, computeBusinessHoursPerDay, computeIsBusinessDate, computeNextBusinessDateStartTime } from "../../src/holidays";
...
describe('computeNextBusinessDateStartTime', () => {
    let holidays:any = [{
        name: '国庆节第一天',
        type: 'public',
        date: new Date("2020-10-01T00:00:00.000Z")
    ...
    it('the start time of next business date of 2020-09-01T07:00:00.000Z is 2020-09-02T01:00:00Z', async () => {
        ...
        let result:any = computeNextBusinessDateStartTime(start, holidays, businessHoursPerDay, businessHours.working_days, utcOffset);
        expect(moment.utc(result).format()).to.be.eq("2020-09-02T01:00:00Z");
    });
    it('the start time of next business date of 2020-09-30T07:00:00.000Z is 2020-10-10T01:00:00Z', async () => {
        ...
        const businessHoursPerDay:any = computeBusinessHoursPerDay(<string>businessHours.start, <string>businessHours.end);
        let result:any = computeNextBusinessDateStartTime(start, holidays, businessHoursPerDay, businessHours.working_days, utcOffset);
        expect(moment.utc(result).format()).to.be.eq("2020-10-10T01:00:00Z");
    });
    ...
    it('the start time of next business date of 2020-10-10T07:00:00.000Z is 2020-10-10T01:00:00Z', async () => {
        ...
        const businessHoursPerDay:any = computeBusinessHoursPerDay(<string>businessHours.start, <string>businessHours.end);
        let result:any = computeNextBusinessDateStartTime(start, holidays, businessHoursPerDay, businessHours.working_days, utcOffset);
        expect(moment.utc(result).format()).to.be.eq("2020-10-11T01:00:00Z");
    });
    it('the start time of next business date of 2020-10-16T07:00:00.000Z is 2020-10-19T01:00:00Z', async () => {
        ...
        const businessHoursPerDay:any = computeBusinessHoursPerDay(<string>businessHours.start, <string>businessHours.end);
        let result:any = computeNextBusinessDateStartTime(start, holidays, businessHoursPerDay, businessHours.working_days, utcOffset);
        expect(moment.utc(result).format()).to.be.eq("2020-10-19T01:00:00Z");
    });
});