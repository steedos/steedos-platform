...
class CompressCommand extends compressOclif.Command {
    async run() {
        try {
            ...
            var tempDir = metadata_core_1.mkTempFolder('compress-');
            ...
            option['cols'] = cols;
            var projectPath = metadata_core_1.resolveProjectPathSync(sourcePath);
            if (!fs.existsSync(sourcePath)) {
                throw new Error('no such file or directory');
            }
            metadata_core_1.compressFiles(sourcePath, sourcePath, tempDir, option, async function (base64) {
