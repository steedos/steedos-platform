...
const split_to_list_1 = require("../../util/split_to_list");
class ExportCommand extends exportOclif.Command {
    async run() {
        try {
            ...
            var ids = split_to_list_1.splitToList(inputIds);
            var fields = split_to_list_1.splitToList(inputFields);
            var params = {
                ids,
                fields,
                recordsCount,
...
function exportDataFromServer(objectName, params, callback) {
    auth_1.authRequest('/api/composite/sobjects/' + objectName, {
        method: "POST",
        headers: {
