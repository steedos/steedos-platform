...
exports.filesTolocalJson = void 0;
...
const loadFileToJson = require('./loadfiletojson').loadFileToJson;
//获取所有layouts
...
async function fieldsToLocalFile(targetPath, object) {
    ...
    for (let key in rawFields) {
        ...
        await fieldToLocal(targetPath, fieldName, field, object).catch(function (err) {
...
async function listViewsToLocalFile(targetPath, object) {
    ...
    for (let key in listViewsName) {
        ...
        await listViewsToLocal(targetPath, name, listView, object).catch(function (err) {
            console.log("listViewsToLOcal err", err);
        });
    }
    return listViews;
}
async function permissionsToLocal(targetPath, permissionName, permission, object) {
    let objectName = object.name;
...
async function permissionsToLocalFile(targetPath, object) {
    ...
    for (let key in permissionsName) {
        ...
        await permissionsToLocal(targetPath, name, permission, object).catch(function (err) {
...
async function buttonsToLocalFile(targetPath, object) {
    ...
    if (buttonsName.length > 0) {
        for (let key in buttonsName) {
            ...
            buttons[name] = button;
            await buttonsToLocal(targetPath, name, button, object).catch(function (err) {
...
async function filesTolocalJson(steedosPackageJson, targetPath) {
    let steedosPackage = {};
    ...
    //console.log(layouts);
    let applications = await applicationsToLocalFile(steedosPackageJson, targetPath);
    let permissionsets = await permissionsetsToLocalFile(steedosPackageJson, targetPath);
...
exports.filesTolocalJson = filesTolocalJson;
//# sourceMappingURL=jsontofiles.js.map