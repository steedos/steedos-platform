...
exports.loadOldFileToJson = void 0;
const load_file_1 = require("../../util/load_file");
const metadata_core_1 = require("@steedos/metadata-core");
const path = require('path');
const fs = require('fs');
const glob = require('glob');
const _ = require('underscore');
var convertFilesJson = [];
var remainFilesJson = [];
...
async function loadApplications(filePath) {
    ...
    for (let i = 0; i < matchedPaths.length; i++) {
        let matchedPath = matchedPaths[i];
        let json = load_file_1.loadFile(matchedPath);
        try {
            if (json) {
                ...
                if (typeof json['actions'] != 'undefined') {
                    ...
                    if (jsPaths.length > 0) {
                        ...
                        for (let key in actions) {
                            ...
                            let element = _.find(jsContent, function (index, name) {
                                return name === key;
                            });
                            let keyVisible = key + "Visible";
                            let elementVisible = _.find(jsContent, function (index, name) {
                                return name === keyVisible;
                            });
                            let elementFn;
                            let elementVisibleFn;
...
function loadObjectActionJs(filePath) {
    let json = load_file_1.loadFile(filePath);
    return json;
}
...
// }
async function loadOldFileToJson(filePath) {
    ...
    const convertTable = new metadata_core_1.Table().createTable(_.sortBy(convertFilesData, 'type'), cols, '已转化文件清单');
    const remainTable = new metadata_core_1.Table().createTable(_.sortBy(remainFilesData, 'type'), cols, '未转化文件清单');
    console.log(convertTable);
    console.log(remainTable);
    return steedosPackage;
}
exports.loadOldFileToJson = loadOldFileToJson;
//# sourceMappingURL=loadfiletojson.js.map