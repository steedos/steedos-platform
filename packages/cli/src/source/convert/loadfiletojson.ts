...
async function loadObjects(filePath) {
  ...
  console.log("loadObjects===>", matchedPaths);
  let results = {};
...
export async function loadOldFileToJson(filePath) {
  ...
  let objects = await loadObjects(filePath);
  ...
  const convertTable = new Table().createTable(
    _.sortBy(convertFilesData, "type"),
    cols,
    "已转化文件清单",
  );
  const remainTable = new Table().createTable(
    _.sortBy(remainFilesData, "type"),
    cols,
    "未转化文件清单",
  );
