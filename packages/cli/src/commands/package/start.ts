...
class StartCommand extends Command {
			...
			await Promise.all(_.uniq(serviceFiles).map(async f => {
				...
				try {
                    ...
                    this.broker.logger.error(
                        kleur.red().bold(`Failed to create service. The file is ${f} and the error: ${error.message}`)
                    );
                }
			}));
		}
  ...
  async run() {
	// console.log(`steedos run....`, StartCommand)
    ...
    this.flags = flags;
    this.servicePaths = _.compact(Object.values(argv))
