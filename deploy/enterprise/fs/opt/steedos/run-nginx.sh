...
if [[ -z "${STEEDOS_ALLOWED_FRAME_ANCESTORS-}" ]]; then
  # https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/frame-ancestors
  export STEEDOS_ALLOWED_FRAME_ANCESTORS="'self'"
else
  ...
  export STEEDOS_ALLOWED_FRAME_ANCESTORS="${STEEDOS_ALLOWED_FRAME_ANCESTORS%;*}"
fi

if [[ -z "${STEEDOS_DISABLE_IFRAME_WIDGET_SANDBOX-}" ]]; then
  # For backwards compatibility, if this is not set to anything, we default to no sandbox for iframe widgets.
  export STEEDOS_DISABLE_IFRAME_WIDGET_SANDBOX="true"
fi
