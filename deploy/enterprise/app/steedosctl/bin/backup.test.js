...
it('Checks for the current Steedos Version.', async () => {
  fsPromises.readFile = jest.fn().mockImplementation(async () => `{"githubRef":"refs/tags/v1.2.3"}`);
  await expect(utils.getCurrentSteedosVersion()).resolves.toBe("v1.2.3")
})

test('If Encryption env values are being removed', () => {
  expect(backup.removeSensitiveEnvData(`STEEDOS_REDIS_URL=redis://127.0.0.1:6379\nSTEEDOS_ENCRYPTION_PASSWORD=dummy-pass\nSTEEDOS_ENCRYPTION_SALT=dummy-salt\nSTEEDOS_INSTANCE_NAME=Steedos\n
  `)).toMatch(`STEEDOS_REDIS_URL=redis://127.0.0.1:6379\nSTEEDOS_INSTANCE_NAME=Steedos\n`)
});

test('If MONGODB env values are being removed', () => {
  expect(backup.removeSensitiveEnvData(`STEEDOS_REDIS_URL=redis://127.0.0.1:6379\nSTEEDOS_MONGODB_URI=mongodb://appsmith:pass@localhost:27017/appsmith\nSTEEDOS_MONGODB_USER=appsmith\nSTEEDOS_MONGODB_PASSWORD=pass\nSTEEDOS_INSTANCE_NAME=Steedos\n
  `)).toMatch(`STEEDOS_REDIS_URL=redis://127.0.0.1:6379\nSTEEDOS_INSTANCE_NAME=Steedos\n`)
});
...
test('Backup Archive Limit when env STEEDOS_BACKUP_ARCHIVE_LIMIT is null', () => {
  expect(backup.getBackupArchiveLimit()).toBe(4)
});

test('Backup Archive Limit when env STEEDOS_BACKUP_ARCHIVE_LIMIT is 5', () => {
