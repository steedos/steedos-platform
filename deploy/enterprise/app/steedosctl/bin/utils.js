...
function showHelp() {
  console.log(
    "\nUsage: steedos <command> to interact with steedos utils tool"
  );
  console.log("\nOptions:\r");
  ...
  console.log("\tbackup\t\t\tTake a backup of Steedos instance.\r");
  console.log("\trestore\t\t\tRestore Steedos instance from a backup.\r");
  ...
  await fsPromises
    ...
    .then((filenames) => {
      for (let filename of filenames) {
        if (filename.match(/^steedos-backup-.*\.tar\.gz$/)) {
...
async function getCurrentSteedosVersion() {
  const githubRef = JSON.parse(await fsPromises.readFile("/opt/steedos/info.json", "utf8")).githubRef;
