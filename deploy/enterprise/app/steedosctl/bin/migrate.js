...
const main = async (userServer, ipServer) => {
  ...
  if (answerConfirm.toLowerCase() === 'y') {
    const folderSsh = `/opt/steedos/.ssh`;
    ...
    if (answerKey.toLowerCase() === 'y') {
      ...
      if (status.code === 0) {
        ...
        const installDir = readlineSync.question('Choose Installation Directory [steedos]: ');

        let installAbsoluteDir = `/home/${userServer}/steedos`;
