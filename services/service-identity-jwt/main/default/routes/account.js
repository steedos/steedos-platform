...
class Account {
    static async ssoLogin(req, res, options = { user: null, err: null, redirect: true, accessToken: null }) {
        ...
        let space = null;
        const accountsServer = await (0, accounts_1.getAccountsServer)();
        const userProfile = await accountsServer.getUserProfile(user.id);
        if (userProfile) {
        ...
        const loginResult = await accountsServer.loginWithUser(user, Object.assign({}, {
            ip,
            userAgent
        }, {
