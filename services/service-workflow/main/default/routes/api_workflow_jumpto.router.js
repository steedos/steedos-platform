 ...
 * @LastEditTime: 2023-05-17 18:18:11
 * @Description: 
 */
'use strict';
// @ts-check
...
/**
@api {post} /api/workflow/jumpto/:instanceId 跳转到审批单
@apiVersion 0.0.0
@apiName /api/workflow/jumpto/:instanceId
...
@apiErrorExample {json} Error-Response:
    ...
    e.message
 */
router.get('/api/workflow/jumpto/:instanceId', core.requireAuthentication, async function (req, res) {
    try {
        ...
        if (ins) {
            ...
            let redirectPath = '';
            if (box) {
                redirectPath = `app/${appCode}/${objectName}/view/${docId}?display=grid&side_object=${objectName}&side_listview_id=${box}`;
            } else {
                redirectPath = `workflow/space/${spaceId}/print/${insId}?box=monitor&print_is_show_traces=1&print_is_show_attachments=1`;
            }
