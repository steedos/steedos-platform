...
router.post("/api/workflow/v2/instance/forward", auth.requireAuthentication, async function (req, res) {
    try {
      ...
      Fiber(async function () {
        try {
          var current_user_info = userSession;
          var current_user_id = current_user_info.userId;

          var hashData = req.body;
          var instance_id = hashData.instance_id;
          var space_id = hashData.space_id;
          var flow_id = hashData.flow_id;
          var hasSaveInstanceToAttachment =
            hashData.hasSaveInstanceToAttachment;
          ...
          common_fields.forEach(function (field) {
            if (field.type == "section") {
                ...
                field.fields.forEach(function (f) {
                  ...
                  if (old_v) {
                    ...
                    if (f.type == "multiSelect") {
                      ...
                      var new_multiSelected = _.intersection(
                        fieldOptions.map(function (n) {
                          return n.value;
                        }),
                        old_multiSelected
                      );
                  ...
                  if (!_.isEmpty(field.fields)) {
                    field.fields.forEach(function (f) {
                      ...
                      if (old_v) {
                        ...
                        // 校验 单选，多选，下拉框 字段值是否在新表单对应字段的可选值范围内
                        if (f.type == "select" || f.type == "radio") {
                          var selectedOption = fieldOptions.find(function (
                            item
                          ) {
                            return item.value === old_v;
                          });
              ...
              // 跨工作区转发不复制选人选组
              if (
                ["group", "user"].includes(field.type) &&
                old_space_id != space_id
              ) {
                return;
              }
          ...
          if (name_forumla) {
            try {
              var iscript = name_forumla
                .replace(/\{/g, "(new_values['")
                .replace(/\}/g, "'] || '')");
              var rev = eval(iscript);
              instance_name = rev || flow.name;
            } catch (error) {
              throw new Meteor.Error(
                "caculate_instance_name",
                "计算申请单标题出错请检查表单标题脚本。"
              );
            }
          } else {
          ...
          if (action_type === "forward") {
            new_values = uuflowManager.getApproveValues(
              new_values,
              start_step.permissions,
              flow.form,
              flow.current.form_version
            );
          }
          ...
          _.each(forward_users, function (user_id) {
            ...
            ins_obj.applicant_organization_fullname =
              space_user_org_info.fullname;
            ins_obj.state = "draft";
            ins_obj.code = "";
            ...
            if (action_type == "distribute") {
              ...
              ins_obj.distribute_from_instances =
                _.clone(ins.distribute_from_instances) || [];
            ...
            if (hasSaveInstanceToAttachment) {
              ...
              instanceHtml = InstanceReadOnlyTemplate.getInstanceHtml(
                user_info,
                space_id,
                ins,
                {
              ...
              instanceFile.attachData(
                Buffer.from(instanceHtml, "utf-8"),
                {
                  type: "text/html",
                },
                function (error) {
