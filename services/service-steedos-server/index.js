...
module.exports = {
	...
	async started() {
		...
		// this.broker.waitForServices(this.name).then(async () => {
			...
			this.broker.emit("steedos-server.started"); //  **已经出掉了waitForServices, 此事件可以作废了, 可使用 dependencies: ['steedos-server']** ; 此处有异步函数，当前服务started后，实际上还未初始化完成。所以此服务初始化完成后，发出一个事件;
		// });
