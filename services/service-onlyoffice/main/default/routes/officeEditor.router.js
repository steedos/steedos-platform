...
const plugins = [];
const cfgSignatureSecret = process.env.STEEDOS_OFFICE_EDITOR_JWT_SECRET
const cfgSignatureSecretExpiresIn = process.env.STEEDOS_OFFICE_EDITOR_JWT_EXPIRES_IN
...
router.get('/api/office/editor/cms_file/:cmsId', auth.requireAuthentication, async function (req, res) {
    try {
        ...
        const data = {
            apiUrl: process.env.STEEDOS_OFFICE_EDITOR_API_URL,
            file: {
                cmsId: cmsId,
                name: fileName,
